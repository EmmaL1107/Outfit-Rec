  ...
  async function handleSave() {
    if (editItem) {
      const updated: ClothingItem = {
        ...editItem,
        ...form,
        image: previewImage || editItem.image,
    ...
    <div className="page wardrobe-page">
      ...
      <div className="filter-bar">
        ...
        <select
          ...
          onChange={(e) => setFilter((f) => ({ ...f, pattern: e.target.value as ClothingPattern | '' }))}
        >
        ...
        <select
          value={filter.scene}
          onChange={(e) => setFilter((f) => ({ ...f, scene: e.target.value as ClothingScene | '' }))}
        >
        ...
        <select
          value={filter.season}
          onChange={(e) => setFilter((f) => ({ ...f, season: e.target.value as Season | '' }))}
        >
        ...
        {hasActiveFilter && (
          <button className="btn btn-ghost" onClick={() => setFilter({ category: '', color: '', thickness: '', style: '', pattern: '', scene: '', season: '' })}>
            清除
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          {hasActiveFilter ? '没有符合条件的衣物' : '衣橱还是空的，快添加第一件衣物吧！'}
        </div>
      ) : (
            ...
            <div key={item.id} className="clothing-card">
              ...
              <div className="clothing-info">
                ...
                <div className="clothing-tags secondary">
                  {item.scene.map((s) => (
                    <span key={s} className="tag tag-scene">{s}</span>
                  ))}
                  {item.season.map((s) => (
                    <span key={s} className="tag tag-season">{s}</span>
                  ))}
                </div>
              ...
              <div className="form-group">
                ...
                <div className="tag-selector">
                  {CLOTHING_CATEGORIES.map((c) => (
                    <button
                      key={c}
                      className={`tag-btn ${form.category === c ? 'active' : ''}`}
              ...
              <div className="form-group">
                ...
                <div className="tag-selector colors">
                  {CLOTHING_COLORS.map((c) => (
                    <button
                      key={c}
                      className={`tag-btn color-btn ${form.color === c ? 'active' : ''}`}
                      style={{ '--btn-color': COLOR_HEX_MAP[c] } as React.CSSProperties}
                      onClick={() => setForm((f) => ({ ...f, color: c }))}
                    >
                      <span className="color-dot" style={{ backgroundColor: COLOR_HEX_MAP[c] }} />
                      {c}
                    </button>
                  ))}
                </div>
              ...
              <div className="form-group">
                ...
                <div className="tag-selector">
                  {CLOTHING_THICKNESSES.map((t) => (
                    <button
                      key={t}
                      className={`tag-btn ${form.thickness === t ? 'active' : ''}`}
                      onClick={() => setForm((f) => ({ ...f, thickness: t }))}
                    >
                      {t}
                    </button>
                  ))}
              ...
              <div className="form-group">
                ...
                <div className="tag-selector">
                  {CLOTHING_STYLES.map((s) => (
                    <button
                      key={s}
                      className={`tag-btn ${form.style === s ? 'active' : ''}`}
              ...
              <div className="form-group">
                ...
                <div className="tag-selector">
                  {CLOTHING_PATTERNS.map((p) => (
                    <button
                      key={p}
                      className={`tag-btn ${form.pattern === p ? 'active' : ''}`}
              ...
              <div className="form-group">
                ...
                <div className="tag-selector">
                  {CLOTHING_SCENES.map((s) => (
                    <button
                      key={s}
                      className={`tag-btn ${form.scene.includes(s) ? 'active' : ''}`}
              ...
              <div className="form-group">
                ...
                <div className="tag-selector">
                  {SEASONS.map((s) => (
                    <button
                      key={s}
                      className={`tag-btn ${form.season.includes(s) ? 'active' : ''}`}
